...
class Search implements ToolInterface
{
    ...
    public function getParametersSchema(): array
    {
        return [
            ...
            'properties' => [
                ...
                'category' => [
                    'type' => 'string',
                    'enum' => $this->categoryMap->getNames((int)$this->storeManager->getStore()->getId()),
                    'description' => 'Top-level storefront department to restrict results to. Use ONLY when the '
                        . 'customer clearly names a department/audience ("for women", "men\'s", "kids") that matches '
                        . 'one of the listed names exactly. Do not guess a department from a product type alone '
                        . '(e.g. "jacket" alone is not a department). Ignored if category_id is also given.',
                ],
                'in_stock_only' => [
                    'type' => 'boolean',
    ...
    public function execute(array $args, ToolContextInterface $context): array
    {
        ...
        if ($categoryId <= 0 && $categoryName !== '') {
            $categoryId = $this->categoryMap->resolveId($categoryName, $context->getStoreId()) ?? 0;
        }
