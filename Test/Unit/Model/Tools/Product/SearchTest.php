...
class SearchTest extends TestCase
{
    ...
    public function testExecuteResolvesNamedCategoryToCategoryId(): void
    {
        [$search, $engineFinder] = $this->makeSearch();
        $engineFinder->method('findByQuery')->willReturnCallback(
            function (string $query, array $boosts, QueryOptionsInterface $options): array {
                $this->assertSame(21, $options->getCategoryId());
                return [];
            }
        );

        $search->execute(['query' => 'jacket', 'category' => 'Women'], $this->makeContext());
    }

    public function testExecuteIgnoresUnresolvableCategoryNameInsteadOfErroring(): void
    {
        [$search, $engineFinder] = $this->makeSearch();
        $engineFinder->method('findByQuery')->willReturnCallback(
            function (string $query, array $boosts, QueryOptionsInterface $options): array {
        ...
        $result = $search->execute(['query' => 'jacket', 'category' => 'Kids'], $this->makeContext());

        $this->assertArrayNotHasKey('error', $result);
    }

    public function testExecutePrefersExplicitCategoryIdOverCategoryName(): void
    {
        [$search, $engineFinder] = $this->makeSearch();
        $engineFinder->method('findByQuery')->willReturnCallback(
            function (string $query, array $boosts, QueryOptionsInterface $options): array {
                $this->assertSame(99, $options->getCategoryId());
                return [];
            }
        );

        $search->execute(['query' => 'jacket', 'category_id' => 99, 'category' => 'Women'], $this->makeContext());
    }
    ...
    private function makeSearch(): array
    {
        ...
        $categoryMap->method('getNames')->willReturn(['Women', 'Men']);
        $categoryMap->method('resolveId')->willReturnMap([
            ['Women', 1, 21],
        ]);
        ...
        return [$search, $engineFinder, $collectionFactory, $priceCurrency, $config, $eavConfig, $logger, $categoryMap];
    }
